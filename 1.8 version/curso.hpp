...
void curso::nuevocurso(curso *cur){
	...
	short comprobacion;
	short pregunta;
	char switcHours;	//Para evitar carácteres invalidos en las horas a al semana
	string cupo;		//Se utiliza para evitar que crasheé a la hora de introducir un caracter en lugar de un número cómo cupo.
	bool notValid;
	...
	do{
		carpeta = "cursos\\";
		do{
			notValid = false;		//Bandera que inicia diciendo que el input de NRC es valido
			cout << "Ingrese el NRC (de 4 a 7 digitos numericos)"<<endl;
			getline(cin, curso_nrc);
			if (isdigit(curso_nrc[0])){			
		  		//cupo_max = stoi (cupo);		//Convierte los digitos antes de cualquier caracter a entero y descarta lso demás  //stoi sólo valido en c++11 
		  		nrccc = atoi(curso_nrc.c_str());	//Version para todos los compiladores
		  	}else{
		  		notValid = true;	
			}
		}while( notValid == true );	//Comprueba que esté bien escrito el NRC -Donaldo
		
		ostringstream convert;   // stream used for the conversion
		convert << nrccc;      // insert the textual representation of 'Number' in the characters in the stream
		curso_nrc = convert.str(); // set 'Result' to the contents of the stream
	...
	do{
		fflush(stdin);
		do{
			cout << "Cantidad de horas por semana(2 / 4): " << endl;
			cout << "1) 2 horas por semana"<<endl;
			cout << "2) 4 horas por semana"<<endl;
			switcHours = getch();
		}while ( !onlyNumbers(switcHours) ); //Si se introduce un carácter invalido (letras o [, /, etc...) vuelve a preguntar
		switch( switcHours ){
			case '1':
				horas = 2;
				break;
	...
	do{
		notValid = false;		//Bandera que inicia diciendo que el input de cupo es valido
		//Se va a repetir si detecta que ingresas un caracter
		cout << "Ingrese el cupo maximo del curso: ";
		getline(cin, cupo);			//Se lee cómo cadena de carácteres
		/*Si el primer carácter es un digito. Podemos tomarlo cómo referencia para convertirlo a string.
			Si se inserta por ejemplo 7asdas. Sólo se tomará como cupo el 7.
			Si se inserta, por ejemplo: a12312. Se pedirá que ingreses un cupo valido.	*/
		if (isdigit(cupo[0])){			
	  		//cupo_max = stoi (cupo);		//Convierte los digitos antes de cualquier caracter a entero y descarta lso demás  //stoi sólo valido en c++11 
	  		cupo_max = atoi(cupo.c_str());	//Version para todos los compiladores
	  	}else{
	  		notValid = true;	
		}
	}while( notValid == true || cupo_max < 0 || cupo_max > 255);		//Only numbers revisa que no metas letras cómo cupo. 
...
void curso::detalles(curso *u){
	...
	if(vacia()){
		...
		if(u->tipo == 1){
			cout << "Extraordinario: Si" << endl;
		}else{
			cout << "Extraordinario: No" << endl;
		}
	...
	if(opcion == 1){//baja
		...
		while (fread(&byte, sizeof(char), sizeof(char), archivo)){
			...
			if(cont > flag && cont <= flag + 9){
				anadir[0] = byte;
				codigo.append(anadir);//SABEMOS QUE AÑADE CORRECTAMENTE EL CODIGO
			}else if(cont > (flag + 9)){
				//finalizo con criterios
				system("cls");
				cout << "AVANCE DE "<< codigo << "\n\n";
				while(nodo->siguiente != NULL){
					cout << setfill(' ') << setw(16) << nodo->nombre << ": "; cin >> entr;
					if(entr > nodo->cantidad){
						entr = nodo->cantidad;
					}else if(entr < 0){
						entr = 0;
					}
...
int alumnos::lista_alumnos(){
	...
	i = 0;
	system("cls");
	cout << "     CODIGO|                Apellidos|    SITUACION|\n";
	if(empty()){
		return i;
	}else{
		while(aux->siguiente != NULL){//recorrer hasta el final
...
void alumnos::consulta(const char* codig){
	...
	if(empty()){
		...
		if(strcmp(aux->codigo.c_str(), codig) == 0){
			...
			cout << "Error en consulta datos ingresados no validos\n" << endl;
